...
with torch.no_grad():
    ...
    if model_exp is not None:
        ...
        learned_exp = torch.tensor([-4.,4.]).to(device).view(1, 1, -1)
    ...
    if model_focal is not None:
        ...
        learned_focal = learned_focal.view(N, -1, 1).repeat(1, vid_dataset.shape[1],  vid_dataset.shape[2])
